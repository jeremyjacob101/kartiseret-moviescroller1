...
import "./MovieScroller3.css";

type FocusPhase3 = "collapsed" | "opening" | "open" | "closing";

type GhostTransitionState3 = {
  itemIndex: number;
  sourceRect: PosterSourceRect3;
  targetRect: PosterSourceRect3;
...
const DETAIL3_REPEAT_SETS = 7;
const DETAIL3_FOCUS_VIEWPORT_PADDING_PX = 28;
const DETAIL3_SCROLL_SETTLE_MS = 110;
const DETAIL3_EDGE_BUFFER_SETS = 1;
const DETAIL3_READY_RADIUS = 2;
const DETAIL3_PRELOAD_RADIUS = 4;
const COLLAPSED_CARD_SCALE_BOOST = 0.15;

const movieScroller3TimingStyle = {
  "--movie-scroller3-stage-fade-duration": `${FOCUS_STAGE_FADE_DURATION_MS}ms`,
  "--movie-scroller3-stage-close-delay": `${CLOSE_STAGE_FADE_DELAY_MS}ms`,
  "--movie-scroller3-focus-poster-fade-duration": `${FOCUS_POSTER_FADE_DURATION_MS}ms`,
  "--movie-scroller3-ghost-move-duration": `${POSTER_MOVE_DURATION_MS}ms`,
  "--movie-scroller3-ghost-opacity-duration": `${POSTER_GHOST_OPACITY_DURATION_MS}ms`,
} as CSSProperties;
...
function mod3(value: number, size: number): number {
  const remainder = value % size;
  return remainder < 0 ? remainder + size : remainder;
}

function easeOutQuad3(value: number): number {
  return 1 - (1 - value) ** 2;
}

function easeOutCubic3(value: number): number {
  return 1 - (1 - value) ** 3;
}

function easeInOutCubic3(value: number): number {
  return value < 0.5
    ? 4 * value ** 3
    : 1 - ((-2 * value + 2) ** 3) / 2;
}

function lerp3(start: number, end: number, progress: number): number {
  return start + (end - start) * progress;
}

function toPosterSourceRect3(
  rect: Pick<DOMRect, "top" | "left" | "width" | "height"> | null | undefined,
  fallback: PosterSourceRect3,
): PosterSourceRect3 {
  if (!rect) {
    return fallback;
  }
...
function getPageScrollTop3(): number {
  return window.scrollY || window.pageYOffset || 0;
}

function getMaxPageScrollTop3(): number {
  const scrollRoot = document.scrollingElement ?? document.documentElement;
  return Math.max(0, scrollRoot.scrollHeight - window.innerHeight);
}

function getTargetDetailViewportTop3(panelHeight: number): number {
  const paddedViewportHeight = Math.max(
    0,
    window.innerHeight - DETAIL3_FOCUS_VIEWPORT_PADDING_PX * 2,
  );
  const centeredHeight = Math.min(panelHeight, paddedViewportHeight);

  return Math.max(
    DETAIL3_FOCUS_VIEWPORT_PADDING_PX,
    (window.innerHeight - centeredHeight) / 2,
  );
}

function getViewportScrollTarget3(
  shellTop: number,
  currentScrollTop: number,
  panelHeight: number,
): number {
  return clamp3(
    currentScrollTop + shellTop - getTargetDetailViewportTop3(panelHeight),
    0,
    getMaxPageScrollTop3(),
  );
}

function getDetailPanelOpacity3(
  itemIndex: number,
  scrollLeft: number,
  clientWidth: number,
  gap: number,
  ...
  return 1 - easeOutQuad3(progress) * 0.16;
}

function buildCardOffset3(
  cardState: MovieScroller3CardState,
  phase: FocusPhase3,
...
function isDetailPanelTarget3(target: EventTarget | null): boolean {
  if (!(target instanceof Element)) {
    return false;
  }

  return Boolean(target.closest(".movie-scroller3-detail-panel"));
}

function getMaxWidthValue3(
...
function getDetailLayout3(
  ...
  const maxWidthValue = getMaxWidthValue3(maxWidth, safeClientWidth);
...
function getCenteredScrollLeft3(
  itemIndex: number,
  clientWidth: number,
  gap: number,
  panelWidth: number,
...
function getCenteredDetailIndex3(
  scrollLeft: number,
  clientWidth: number,
  gap: number,
  panelWidth: number,
...
export function MovieScroller3({
  cardWidth = 240,
  cardHeight = 360,
  gap = 16,
  maxWidth = "100%",
  ...
  const detailMiddleStartIndex =
    Math.floor(DETAIL3_REPEAT_SETS / 2) * movieCount;
  const detailTotalItems = DETAIL3_REPEAT_SETS * movieCount;
  const collapsedHeight = Math.ceil(
    cardHeight * (1 + COLLAPSED_CARD_SCALE_BOOST),
  );

  const [phase, setPhase] = useState<FocusPhase3>("collapsed");
  ...
  const [ghostTransition, setGhostTransition] =
    useState<GhostTransitionState3 | null>(null);

  const shellRef = useRef<HTMLDivElement | null>(null);
  const detailScrollerRef = useRef<HTMLDivElement | null>(null);
  const posterRef = useRef<HTMLImageElement | null>(null);
  const ghostRef = useRef<HTMLImageElement | null>(null);
  const targetRectRef = useRef<PosterSourceRect3 | null>(null);
  ...
  const activeMovieIndex = mod3(detailActiveIndex, movieCount);
  const detailLayout = getDetailLayout3(
    detailClientWidth,
    gap,
    maxWidth,
    detailTotalItems,
  ...
  const applyRectToGhost = useCallback((rect: PosterSourceRect3) => {
  ...
  const applyGhostOpeningAppearance = useCallback((progress: number) => {
    ...
    ghost.style.borderRadius = `${lerp3(
  ...
  const getCurrentDestinationRect = useCallback(
    (itemIndex: number, fallback: PosterSourceRect3) => {
      const item = shellRef.current?.querySelector<HTMLElement>(
        `[data-scroller3-item-index="${itemIndex}"]`,
      );
  ...
  const scrollDetailToIndex = useCallback(
    (itemIndex: number, behavior: ScrollBehavior = "auto") => {
      ...
      const layout = getDetailLayout3(
        clientWidth,
        gap,
        maxWidth,
        detailTotalItems,
      );
      const centeredScrollLeft = getCenteredScrollLeft3(
        itemIndex,
        clientWidth,
        gap,
        layout.panelWidth,
  ...
  const syncDetailViewportToFocusPosition = useCallback(
    (behavior: ScrollBehavior = "auto") => {
      ...
      const currentScrollTop = getPageScrollTop3();
      const panelHeight = getDetailLayout3(
        measureDetailScroller(),
        gap,
        maxWidth,
        detailTotalItems,
      ).panelHeight;
      const targetScrollTop = getViewportScrollTarget3(
  ...
  const getCenteredIndexFromScroller = useCallback(() => {
    ...
    return getCenteredDetailIndex3(
      scroller.scrollLeft,
      clientWidth,
      gap,
      layout.panelWidth,
  ...
  const recenterDetailIndex = useCallback(
    (itemIndex: number) => {
      ...
      const minSet = DETAIL3_EDGE_BUFFER_SETS;
      const maxSet = DETAIL3_REPEAT_SETS - DETAIL3_EDGE_BUFFER_SETS - 1;

      if (setIndex > minSet && setIndex < maxSet) {
        return itemIndex;
      }

      const centeredIndex = detailMiddleStartIndex + mod3(itemIndex, movieCount);
  ...
  const scheduleSettledCommit = useCallback(() => {
    ...
    detailSettleTimeoutRef.current = window.setTimeout(() => {
      ...
      detailSettleTimeoutRef.current = null;
    }, DETAIL3_SCROLL_SETTLE_MS);
  }, [commitCenteredDetailIndex]);

  const handleSelectCollapsedMovie = useCallback<
    NonNullable<MovieScroller3Props["onSelectMovie"]>
  >(
    (_movie, sourceRect, itemIndex, sourceOpacity = 1) => {
      ...
      const movieIndex = mod3(itemIndex, movieCount);
  ...
  useLayoutEffect(() => {
    ...
    const openingPanelHeight = getDetailLayout3(
      measureDetailScroller(),
      gap,
      maxWidth,
      detailTotalItems,
    ).panelHeight;
    const initialScrollTop = getPageScrollTop3();
    const targetScrollTop = getViewportScrollTarget3(
      shell.getBoundingClientRect().top,
      initialScrollTop,
      openingPanelHeight,
    );
    const initialTargetRect = toPosterSourceRect3(
    ...
    animationFrameRef.current = window.requestAnimationFrame((startTime) => {
      ...
      const animateOpening = (frameTime: number) => {
        const linearProgress = clamp3(
          (frameTime - startTime) / POSTER_MOVE_DURATION_MS,
          0,
          1,
        );
        const scrollProgress = easeInOutCubic3(linearProgress);
        const ghostProgress = easeOutCubic3(linearProgress);
        const nextScrollTop = lerp3(
          initialScrollTop,
          targetScrollTop,
          scrollProgress,
        );
        ...
        const liveTargetRect = toPosterSourceRect3(
          posterRef.current?.getBoundingClientRect(),
        ...
        applyRectToGhost({
          top: lerp3(ghostTransition.sourceRect.top, liveTargetRect.top, ghostProgress),
          left: lerp3(
  ...
  useEffect(() => {
    ...
    for (
      let offset = -DETAIL3_PRELOAD_RADIUS;
      offset <= DETAIL3_PRELOAD_RADIUS;
      offset += 1
    ) {
      preloadMovieIndexes.add(mod3(activeMovieIndex + offset, movieCount));
    }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowRight") {
        ...
        handleFocusDetailMovie(clamp3(detailActiveIndex + 1, 0, detailTotalItems - 1));
        return;
      }
  ...
  const getCardStyle = useCallback(
    (cardState: MovieScroller3CardState) => {
      const style = buildCardOffset3(cardState, phase);
  ...
  const handleDetailLayerClick = useCallback(
    (event: MouseEvent<HTMLDivElement>) => {
      if (phase !== "open" || isDetailPanelTarget3(event.target)) {
        return;
      }
  ...
  const detailPanels = Array.from({ length: detailTotalItems }, (_, itemIndex) => {
    const movie = movies[mod3(itemIndex, movieCount)];
    const isActive = itemIndex === visualActiveIndex;
    const distanceFromActive = Math.abs(itemIndex - visualActiveIndex);
    const isDetailReady = distanceFromActive <= DETAIL3_READY_RADIUS;
    const detailOpacity = getDetailPanelOpacity3(
      itemIndex,
      detailScrollLeft,
      detailClientWidth,
      gap,
    ...
    return (
      <article
        ...
        className={`movie-scroller3-detail-panel${isActive ? " is-active" : ""}${
      ...
      >
        {isDetailReady && movie.backdropSrc ? (
          <div className="movie-scroller3-detail-backdrop-shell" aria-hidden="true">
            <img
              src={movie.backdropSrc}
              alt=""
              className="movie-scroller3-detail-backdrop"
              decoding="async"
        ...
        <div className="movie-scroller3-detail-sheen" aria-hidden="true" />
        ...
        {isDetailReady ? (
          <>
            {isActive ? (
              <button
                type="button"
                className="movie-scroller3-close"
              ...
              <button
                type="button"
                className="movie-scroller3-detail-focus-overlay"
            ...
            <div className="movie-scroller3-detail-content">
              <MovieDetailsContent
                movie={movie}
                posterRef={isActive ? posterRef : undefined}
                titleId={`${titleId}-${itemIndex}`}
                eyebrow="Now playing"
                posterClassName={`details-poster movie-scroller3-detail-poster${
                  isActive
          ...
          <button
            type="button"
            className="movie-scroller3-detail-preview"
            aria-label={`Focus ${movie.title}`}
            onClick={() => {
              handleFocusDetailMovie(itemIndex);
            }}
          >
            <img
              src={movie.imageSrc}
              alt={movie.title}
              className="movie-scroller3-detail-preview-image"
  ...
  return (
    <div
      ...
      style={{
        ...movieScroller3TimingStyle,
        height: phase === "collapsed" ? collapsedHeight : detailLayout.panelHeight,
      }}
    >
      <div className="movie-scroller3-collapsed-layer" aria-hidden={isDetailMounted}>
        <MovieScrollerBase3
          cardWidth={cardWidth}
          ...
          className="movie-scroller3-collapsed"
        />
      </div>

      {isDetailMounted ? (
        <div
          className="movie-scroller3-detail-layer"
          onClick={handleDetailLayerClick}
        >
          <div
            ref={detailScrollerRef}
            className="movie-scroller3-detail-rail"
            onScroll={handleDetailScroll}
          >
            <div
              className="movie-scroller3-detail-track"
              style={{
                height: detailLayout.panelHeight,
      ...
      {showGhost && ghostTransition ? (
        <img
          ref={ghostRef}
          src={movies[mod3(ghostTransition.itemIndex, movieCount)].imageSrc}
          alt=""
          aria-hidden="true"
          className={`movie-scroller3-poster-ghost${
            phase === "opening" ? " is-opening" : ""
